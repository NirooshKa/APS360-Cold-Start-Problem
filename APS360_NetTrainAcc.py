...
class FCTestNN(nn.Module):
    def __init__(self, class_size):
        ...
        self.name = "FCTestNN"
        self.fc1 = nn.Linear(3*224*224, 256) #(RGB incoming pixels, arbitrary node number)
...
class ConvTestNN(nn.Module):
    def __init__(self, class_size):
        ...
        self.fc1 = nn.Linear(5*110*110, class_size)
...
def train_net(model, train_data, val_data, batch_size=4, num_epochs=1, learning_rate=0.001, momentum=0.9, use_cuda=False, num_iters=10):
    ...
    print("The total training time was: {0} seconds".format(end_time - start_time))
